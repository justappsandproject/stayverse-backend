...
class BrimPusher {
  ...
  static Future<ServerResponse?> pushToken({bool? notificationsEnabled}) async {
    try {
      ...
      final bool isNotificationsEnabled = notificationsEnabled ??
          BrimAuth.curentUser()?.notificationsEnabled ??
          true;

      final response = await locator.get<Dio>().patch<DynamicMap>(
          '${Constant.host}/agents/notifications/device-token',
          data: {'deviceToken': deviceToken, 'enable': isNotificationsEnabled});
