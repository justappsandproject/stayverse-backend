...
class PendingApartmentDetails extends ConsumerStatefulWidget {
  static const route = '/PendingApartmentDetails';
  const PendingApartmentDetails({super.key, required this.apartmentDetails});
  final Booking? apartmentDetails;

  @override
  ConsumerState<PendingApartmentDetails> createState() =>
      _PendingApartmentDetailsState();
}

class _PendingApartmentDetailsState
    extends ConsumerState<PendingApartmentDetails> {
      ...
      body: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            24.sbH,
            PaymentInfoCard(
              booking: widget.apartmentDetails,
            ),
            30.sbH,
            ...
            25.sbH,
            TimelineWidget(
              checkedInString: 'Check In',
              checkedOutString: 'Check Out',
            ...
            AppBtn.from(
              ...
              onPressed: () {
                _submit(status: BookingStatus.accepted);
              },
            ),
            10.sbH,
            AppBtn.from(
  ...
  _submit({required BookingStatus status}) async {
    ...
    if (status == BookingStatus.accepted) {
      success = await controller.acceptBooking(request);
