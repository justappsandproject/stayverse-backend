...
import '../ui_state/image_picker_ui_state.dart';

class ImagePickerWidget extends ConsumerWidget {
  final ProviderMode mode;
  const ImagePickerWidget({super.key, required this.mode});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final apartmentState = ref.read(apartmentAdvert);
    final isValid = images.length >= 2 && images.length <= 12;
    final isAtMaximum = images.length >= 12;

    if (apartmentState.apartmentImages?.isNotEmpty == true && images.isEmpty) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        ref.read(imageProvider.notifier).setApartmentImages(
              apartmentState.apartmentImages?.cast<ImageFile>() ?? [],
            );
      });
    }
