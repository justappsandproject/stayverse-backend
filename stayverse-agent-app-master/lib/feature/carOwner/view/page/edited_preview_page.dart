...
import 'package:stayvers_agent/feature/carOwner/controller/edit_ride_controller.dart';
...
class EditedPreviewPage extends ConsumerStatefulWidget {
  static const route = '/EditedPreviewPage';
  final String? rideId;
  const EditedPreviewPage({super.key, this.rideId});

  @override
  ConsumerState<EditedPreviewPage> createState() => _EditedPreviewPageState();
}

class _EditedPreviewPageState extends ConsumerState<EditedPreviewPage> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ref.read(editRideAdvertProvider.notifier).updateStateFromControllers();
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    final formData = ref.watch(editRideAdvertProvider);
    final formattedPrice = formData.pricePerHour! > 0
        ...
        : 'N/A';
    return BrimSkeleton(
      isBusy: ref.watch(editRideController).isBusy,
      appBar: AppBar(
                    ...
                    itemBuilder: (context, index) {
                      return ClipRRect(
                        ...
                        child: formData.rideImages != null
                            ? (formData.rideImages![index].isRemote
                                ? Image.network(
                                    formData.rideImages![index].path,
                                    fit: BoxFit.cover,
                                    width: 160,
  ...
  void _submit(BuildContext context, WidgetRef ref) async {
    ...
    final formData = ref.read(editRideAdvertProvider);
    final controller = ref.read(editRideController.notifier);
    ...
    // Submit to backend
    final proceed = await controller.editRide(widget.rideId ?? '', processedRequest);

    if (proceed) {
      ref.read(editRideAdvertProvider.notifier).resetAllFormData(ref);
