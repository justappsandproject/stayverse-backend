...
import 'car_details_form_page.dart';
import 'ride_amenities_selection.dart';

class CreateCarAdvert extends ConsumerStatefulWidget {
  const CreateCarAdvert({super.key});

  @override
  ConsumerState<CreateCarAdvert> createState() => _CreateCarAdvertState();
}

class _CreateCarAdvertState extends ConsumerState<CreateCarAdvert> {
  @override
  Widget build(BuildContext context) {
    final state = ref.watch(createVehicleType);
          ...
          children: [
            ...
            Column(
              ...
              children: [
                ...
                10.sbH,
                const RideLocationSearch(mode: ProviderMode.create),
              ],
            ),
            27.sbH,
            AppDropdown(
              ...
              onChanged: (selectedType) {
                ref.read(createVehicleType.notifier).selectType(selectedType!);
                ref
                    .read(createRideAdvertProvider.notifier)
                    .updateRideType(selectedType);
              },
            ),
            27.sbH,
            const RideImagePickerWidget(mode: ProviderMode.create),
            27.sbH,
            const RideAmenitiesSelection(mode: ProviderMode.create),
            27.sbH,
            Align(
...
void _submit(WidgetRef ref) {
  final notifier = ref.read(createRideAdvertProvider.notifier);
  final selectedAmenities = ref
      .read(createRideAmenitiesProvider)
      .where((amenity) => amenity.isSelected)
      .toList();
  final selectedImages = ref.read(createRideImagePickerProvider);
  ...
  if (location.isEmpty) {
    BrimToast.showHint('Please enter your Ride location.');
    return;
  }

  if (isAmenitiesValid && isImagesValid) {
    $navigate.to(CarDetailsFormPage.route);
  } else {
    BrimToast.showHint(
